...
public class WeatherEffect extends AbstractGameEffect {
    private float x;
    private float y;
    private float vY;
    ...
    public WeatherEffect(String WeatherId) {
        this.img = ImageMaster.EXHAUST_L;
        this.duration = 2.0F;
        this.scale = MathUtils.random(2.7F, 2.5F) * Settings.scale;
        if (WeatherId.equals("Storm")) {
            this.color = new Color(MathUtils.random(0.9F, 1.0F), MathUtils.random(0.9F, 1.0F), MathUtils.random(0.6F, 0.8F), 0.0F);
        } else if (WeatherId.equals("SuperRobot:SnowStorm")) {
